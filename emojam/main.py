...
class EmojamWindow(Gtk.Window):
    def __init__(self):
        ...
        self.s_active_group = ""    # to keep the toggled init from complaining on startup
        ...
        self.refresh_statusbar()    # in case the config file overrides the default
    ...
    def make_larger_layout(self):
        ...
        search_line_box.pack_start(hamburger_menu, expand=False, fill=True, padding=0)
        search_line_box.pack_start(search_box, expand=True, fill=True, padding=0)
        search_line_box.pack_start(zoom_control_box, expand=False, fill=True, padding=0)
        # add search box to the top of the VBox
        box_layout.pack_start(search_line_box, expand=False, fill=True, padding=0)
        # make output line
        self.output_line_box = self.make_output_line()
        box_layout.pack_start(self.output_line_box, expand=False, fill=True, padding=0)
        ...
        box_layout.pack_start(scrolled_flowbox, expand=True, fill=True, padding=0)
        ...
        #statusbar_label = message_area.get_children()
        #print(statusbar_label)
        #message_area.set_child_packing(statusbar_label[0], False, False, 0, Gtk.PackType.START)
        box_layout.pack_end(self.status_bar, expand=False, fill=False, padding=0)
        return box_layout
    ...
    def make_hamburger_menu(self):
        ...
        menu_button.add_accelerator("activate", self.accel_group, key, mod, Gtk.AccelFlags.VISIBLE)
        # Make menu
        menu = Gtk.Menu()
        ...
        #favorites_menu_item.connect_data("activate", self.jump_to_favorites, "Favorites")
        favorites_menu_item.show()
        # give it a keyboard shortcut
        key, mod = Gtk.accelerator_parse("<Control>f")
        favorites_menu_item.add_accelerator("activate", self.accel_group, key, mod, Gtk.AccelFlags.VISIBLE)
        ...
        self.statusbar_checkbox_menu_item.connect_object('toggled', self.toggled_statusbar, \
        ...
        self.statusbar_checkbox_menu_item.add_accelerator("activate", self.accel_group, key, mod, Gtk.AccelFlags.VISIBLE)
        ...
        self.zoom_checkbox_menu_item.connect_object('toggled', self.toggled_zoom_controls, \
        ...
        exit_menu_item.add_accelerator("activate", self.accel_group, key, mod, Gtk.AccelFlags.VISIBLE)
    ...
    def toggled_statusbar(self, statusbar_checkbox_menu_item):
        ...
        if self.status_bar:  # tricky again
    ...
    def make_output_line(self):
        ...
        #label_output.set_alignment(0.0, 0.0)
    ...
    def make_search_box(self):
        ...
        self.search_entry.connect("changed", self.search_box_changed, self.search_entry)
    ...
    def make_emoji_button_context_menu(self):
        ...
        favorites_menu_item.show()
        return menu, favorites_menu_item    # I promised myself I'd never love twice.
    ...
    def toggled_favorites(self, menu_item):
        ...
        if self.s_active_group == "Favorites":
            self.flowbox.set_filter_func(self.flowbox_group_filter, self.s_active_group)
    ...
    def make_group_button(self, s_button_name: str):
        ...
        button.connect_object("event", self.button_mouse_event, button) # right click and hover
        self.group_buttons_dict.update({s_button_name: button}) # map name to widget
        return button
    ...
    def make_zoom_control_box(self):
        ...
        #frame = Gtk.AspectFrame(label = "—", xalign = 0.5, yalign = 0.5, ratio = 5.0, obey_child = False)
        #frame = Gtk.AspectFrame(label = "—", ratio = 1.0, obey_child = False)
        box = Gtk.Box()
        box.pack_start(label, expand=False, fill=True, padding=5)
        #box.pack_start(frame, expand=False, fill=True, padding=5)
        #zoom_out_event_box.add(label)
        zoom_out_event_box.add(box)
        #zoom_out_icon = Gtk.Image.new_from_icon_name("zoom-out", Gtk.IconSize.LARGE_TOOLBAR)
        #zoom_out_icon.show()
        #zoom_out_event_box.add(zoom_out_icon)
        zoom_out_event_box.set_name('-')
        zoom_out_event_box.connect_object("button_press_event", self.zoom_button_clicked, zoom_out_event_box)
        zoom_out_event_box.connect_object("event", self.button_mouse_event, zoom_out_event_box) # right click and hover
        zoom_out_event_box.show()
        zoom_in_event_box = Gtk.EventBox()
        #label = Gtk.Label(label='+')
        #zoom_in_event_box.add(label)
        label = Gtk.Label(label='+') 
        box = Gtk.Box()
        #frame = Gtk.AspectFrame(label = "+", xalign = 0.5, yalign = 0.5, ratio = 1.0, obey_child = False)
        box.pack_start(label, expand=False, fill=True, padding=5)
        #box.pack_start(frame, expand=False, fill=True, padding=5)
        #zoom_out_event_box.add(label)
        ...
        #zoom_in_icon.show()
        #zoom_in_event_box.add(zoom_in_icon)
        zoom_in_event_box.set_name('+')
        zoom_in_event_box.connect_object("button_press_event", self.zoom_button_clicked, zoom_in_event_box)
        zoom_in_event_box.connect_object("event", self.button_mouse_event, zoom_in_event_box) # right click and hover
        ...
        zoom_size_event_box.connect_object("button_press_event", self.zoom_button_clicked, zoom_size_event_box)
        zoom_size_event_box.connect_object("event", self.button_mouse_event, zoom_size_event_box) # right click and hover
        zoom_size_event_box.show()
        # put them in a box, with separators
        zoom_controls_box = Gtk.HBox()       
        zoom_controls_box.pack_start(zoom_out_event_box, expand=False, fill=True, padding=0)
        separator = Gtk.Separator(orientation=Gtk.Orientation.VERTICAL)
        separator.show()
        zoom_controls_box.pack_start(separator, expand=False, fill=True, padding=0)
        zoom_controls_box.pack_start(zoom_size_event_box, expand=False, fill=True, padding=0)
        separator = Gtk.Separator(orientation=Gtk.Orientation.VERTICAL)
        separator.show()
        zoom_controls_box.pack_start(separator, expand=False, fill=True, padding=0)
        zoom_controls_box.pack_start(zoom_in_event_box, expand=False, fill=True, padding=0)
    ...
    def enter_hover_over_button(self, button):
        ...
        #print(f"Entering hover over {s_emoji_name}")
    ...
    def leave_hover_over_button(self, button):
        ...
        #print(f"Leaving hover over {s_emoji_name}")
        button_style_context = button.get_style_context()
    ...
    def emoji_button_mouse_event(self, widget, event, button):  # widget is the menu widget
        #print(f"Debug: emoji_button_mouse_event, button: %i" % (event.button.button))
        #print(f"Debug: emoji_button_mouse_event, button: {event}")
        if event.type == Gdk.EventType.ENTER_NOTIFY:
    ...
    def flowbox_search_filter(self, fb_child, text):
        ...
        for button in fb_child:
            if text in button.get_name().lower():
                return True
            #elif self.emo.name_from_emoji(text) in button.get_name().lower():
            #    return True
    ...
    def make_emoji_button(self, s_emoji_name: str):
        ...
        button.connect_object("event", self.emoji_button_mouse_event, menu, button) # right click and hover
    ...
    def set_css_style(self):
        ...
        #selected_bg_color.color.alpha = 0.7
    ...
    def clicked_group_button(self, widget, event):
        ...
        #s_group: str = widget.get_tooltip_text()
    ...
    #def clicked_emoji_button(self, widget, event):
